...
export default function KPISection({ kpis = {}, trend = [] }) {
  ...
  return (
    <section id="dashboard-overview" className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
      {KPI_META.map(([key, label, Icon, tone], index) => (
        <motion.article
          key={key}
          className="rounded-xl border border-slate-800 bg-slate-900/80 p-5 shadow-xl shadow-slate-950/20 transition hover:border-cyan-400/30 hover:shadow-cyan-950/20"
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-3xl font-black text-slate-50">{formatValue(key, kpis[key])}</p>
            </div>
            <span className={`rounded-lg border border-slate-700 bg-slate-950/70 p-2 ${tone}`}>
          ...
          <p className="mt-3 text-xs text-slate-500">{Number(kpis[key] || 0) >= 75 ? "Trending healthy" : "Needs attention"}</p>
        </motion.article>
      ))}
    </section>
  );
